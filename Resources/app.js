(() => {
  ...
  const platformCopy = value => {
    ...
    return String(value)
      ...
      .replaceAll("dans le Finder", "dans l’Explorateur de fichiers")
      .replaceAll("dans Finder", "dans l’Explorateur de fichiers")
      .replaceAll("dossiers Finder", "dossiers de l’Explorateur de fichiers")
      .replaceAll("dossier Finder", "dossier de l’Explorateur de fichiers")
  ...
  function applyLanguage(value){
    ...
    const staticCopy = [
      ...
      [".brand-copy span", "Centre de pilotage IA"],
      [".shortcuts-button span", "Raccourcis clavier"]
    ];
    for(const [selector,copy] of staticCopy){const element=document.querySelector?.(selector);if(element)element.textContent=t(copy)}
    const sidebarToggle=document.querySelector?.(".sidebar-toggle");
    if(sidebarToggle){sidebarToggle.title=t("Replier le menu");sidebarToggle.setAttribute("aria-label",t("Replier le menu"))}
    const syncStack=document.querySelector?.(".sidebar-sync-stack");
    if(syncStack)syncStack.setAttribute("aria-label",t("Actualisation des conversations"));
    const resizer=document.querySelector?.("[data-sidebar-resize]");
    if(resizer)resizer.title=t("Glisser pour ajuster · double-clic pour réinitialiser");
    const utility=document.querySelector?.("#utility-panel");
    if(utility)utility.setAttribute("aria-label",t("Panneau droit"));
  }
  ...
  const builtinEngines = [
    ...
    ["claude-code","Claude",[["default","Automatique (recommandé)"],["opus","Claude Opus"],["sonnet","Claude Sonnet"],["haiku","Claude Haiku"]],"default"],
  ];
  ...
  const notificationEventCatalog = [
    ["taskComplete","Résultat prêt","Une tâche Codex ou Claude a terminé avec succès.","check"],
    ...
    ["chatReply","Réponse du chat latéral","Une réponse Codex ou Claude est arrivée dans le panneau droit.","chat"],
  ...
  function renderFirstRun(){
    if(!board.spaces.length){
      const agentState=engineAvailability.checked
        ? `<div class="first-run-agents"><span class="${engineAvailability.codex?"ready":"missing"}">${engineAvailability.codex?icon("check"):icon("alert")} Codex · ${t(engineAvailability.codex?"détecté":"introuvable")}</span><span class="${engineAvailability.claude?"ready":"missing"}">${engineAvailability.claude?icon("check"):icon("alert")} Claude · ${t(engineAvailability.claude?"détecté":"introuvable")}</span></div>`
        : `<div class="first-run-agents checking">${icon("sync")} ${t("Détection de Codex et Claude…")}</div>`;
      const steps=[[t("Choisis un dossier"),t("CTRL KANB crée le projet sans déplacer ni modifier son contenu.")],[t("Confie une tâche"),t("Sélectionne Codex ou Claude, puis décide de son accès au dossier.")],[t("Relis le résultat"),t("La conversation, les validations et l’historique restent réunis dans la carte.")]];
  ...
  function flowLateReason(card){
    if(card.scheduleState==="skipped")return t("Créneau manqué");
    ...
    if(card.launchMode==="scheduled"&&Number.isFinite(scheduled)&&scheduled<nowMs-flowLateGraceMs&&["backlog","ready"].includes(card.status)){
      ...
      return days>=1?t("Lancement en retard de {n} j",{n:days}):t("Lancement dépassé");
    }
  ...
  function renderFlowSection({id,tone="",kicker,title,description,cards,note}){
    ...
    return `<section class="flow-dashboard-section ${tone}"><div class="flow-dashboard-head"><div><span class="section-kicker">${esc(t(kicker))}</span><h2>${esc(t(title))}</h2><p>${esc(t(description))}</p></div><strong>${cards.length}</strong></div><div class="flow-task-list">${visible.map(card=>renderFlowItem(card,note?note(card):"")).join("")}${remaining?`<button class="flow-dashboard-more" data-action="show-more-flow" data-flow-key="${esc(id)}" data-current-limit="${limit}">${esc(tn(remaining,"Afficher {n} tâche de plus","Afficher {n} tâches de plus"))}</button>`:""}</div></section>`;
  }
  function renderFlowTier({id,title,description,clear,sections}){
    const body=sections.map(renderFlowSection).join("");
    return `<section class="flow-tier ${esc(id)}"><div class="flow-tier-head"><h2>${esc(t(title))}</h2><p>${esc(t(description))}</p></div>${body?`<div class="flow-dashboard-grid">${body}</div>`:`<div class="flow-tier-clear">${icon("check")}<span>${esc(t(clear))}</span></div>`}</section>`;
  }
  ...
  function renderSettingsPage(){
    ...
    return `<div class="settings-shell"><main class="settings-content" id="settings-content"><form id="preferences-form" class="settings-page-form">
      ...
      <section id="settings-about" class="preference-section about-section"><img src="AppIcon.png" alt="Icône CTRL KANB"><div><h2>CTRL KANB</h2><p>${t("Version")} ${esc(appVersion||"—")}</p><small>${t("Application macOS locale pour piloter Codex et Claude.")}</small></div></section>
...
<details class="form-section" ${assignedValues(contentCard).length?"open":""}><summary>${t("Classement")} <small>${t("Objectifs, sous-projets et étiquettes")}</small></summary><div class="taxonomy-grid">${categoryFields(contentCard)}</div><div class="field"><label>${t("Étiquettes libres")}</label><input name="labels" value="${esc((contentCard?.labels||[]).join(", "))}" placeholder="${t("Séparées par des virgules")}"></div><div class="field"><label>${t("Numéro dans la priorité")}</label><input type="number" min="1" name="priorityNumber" value="${esc(contentCard?.priorityNumber||"")}" placeholder="${t("Attribué automatiquement")}"></div></details>
  ...
  function claudeSyncStatusCopy(){
    ...
    return {label:board.settings.autoSync?t("Actualisation automatique Claude"):t("Actualiser Claude"),detail:t("Relit via Claude Code CLI les sessions locales liées, sans envoyer de message.")};
  }
  ...
  function renderOneSyncStatus(selector,state,busy,copy,engine){
    ...
    const status=sidebarSyncStateLabel(engine,state),label=engine==="codex"?"Codex":"Claude",statusIcon=busy?"sync":["error","partial"].includes(state.phase)?"alert":state.phase==="success"?"check":"sync";
  ...
  function renderSyncStatus(){
    ...
    if(button){button.disabled=busy;button.classList.toggle("is-busy",busy);button.title=t("Relire les conversations Codex et Claude liées");const label=button.querySelector("span");if(label)label.textContent=t(busy?"Actualisation en cours…":"Actualiser maintenant");}
  ...
  function syncAllNow(silent=false){syncNow(true);syncClaudeNow(true);if(!silent)toast(t("Synchronisation de Codex et Claude en cours…"),false,"essential");}
  ...
  function rememberRunConnection(engine,success,rawFailure){
    const key=engineKey(engine),account=activeAccountID(key),detail=errorText(rawFailure),at=now();
    let state="";
    if(success||quotaError(detail))state="ready";
    else if(/401|oauth|access token.*(?:revoked|expired)|token.*(?:revoked|expired)|not logged in|login required|unauthorized|authentication required|auth login|connexion.*expir/i.test(detail))state="blocked";
    if(!state)return;
    const result={engine:key,account,state,at,detail:state==="blocked"?friendlyAgentError(detail,key):""};
    engineProbes={...engineProbes,[account]:result};
    board.settings.accountChecks={...(board.settings.accountChecks||{}),[account]:result};
  }
  ...
  const commandActions=()=>[
    ...
    {label:t("Chat du projet"),detail:t("Échanger avec Codex ou Claude"),key:"⌥⌘C",icon:"chat",run:()=>openUtilityPanel("chat")},
    ...
    {label:t("Raccourcis clavier"),detail:t("Tout à portée de main"),key:"?",icon:"terminal",run:()=>openShortcuts()}
  ];
  ...
  function accountsSectionMarkup(){
    return `<section id="settings-accounts" class="preference-section accounts-settings-section"><div class="preference-heading"><h2>${t("Comptes utilisés")}</h2><p>${t("Choisis séparément le compte employé par Codex et par Claude.")}</p></div><div class="account-memory-note">${icon("shield")}<div><strong>${t("Ce que CTRL KANB mémorise")}</strong><p>${t("Le compte actif, son dossier de configuration et le résultat du dernier test. Les identifiants restent gérés par les outils locaux dans leurs propres dossiers.")}</p></div></div>${builtinEngines.map(([engine,label])=>{
  ...
  function engineSettingsMarkup(){
    const engines=[
      ...
      ["claude-code","Claude",engineAvailability.claude,t("Connexion locale via Claude Code CLI · sessions, permissions et reprises")],
    ];
    return engines.map(([id,label,ready,detail])=>{
      ...
      const modelHelp=id==="claude-code"?t("Automatique laisse Claude Code CLI choisir un modèle disponible pour ce compte."):t("Utilisé par les nouvelles tâches {agent}.",{agent:label});
  ...
  function enhanceSidebarEngines(){
    ...
    if(engineNode)engineNode.innerHTML=[["codex","Codex"],["claude-code","Claude"]].map(([engine,label])=>{
