(() => {
  ...
  const friendlyUtilityError = value => /already has an active writer/i.test(String(value||"")) ? utilityActiveWriterMessage() : String(value||"");
  ...
  const accountRoot = "~/Library/Application Support/CTRL KANB/Comptes";
  ...
  function migrate(data) {
    ...
    for(const [id,check] of Object.entries(data.settings.accountChecks)){
      ...
      cleanAccountChecks[id]={engine:engineKey(check.engine||id.split(':')[0]),account:id,state:check.state,at:String(check.at||""),detail:check.state==="ready"?"":String(check.detail||"").slice(0,500)};
    }
    ...
    for (const card of data.cards) {
      ...
      for(const conversation of card.conversations){if(!conversation.engine){conversation.engine=normalizeEngine(card.agentEngine);changed=true;}if(!conversation.accountID){const key=engineKey(conversation.engine),chosen=data.settings.activeAccount[key];conversation.accountID=validAccountIDs.has(chosen)?chosen:`${key}:default`;changed=true;}}
    ...
    data.utilityChats=data.utilityChats.filter(chat=>{
      ...
      for(const conversation of chat.conversations){if(!conversation.engine){conversation.engine=engine;changed=true;}if(!conversation.accountID){const chosen=data.settings.activeAccount[engine];conversation.accountID=validAccountIDs.has(chosen)?chosen:`${engine}:default`;changed=true;}conversation.messages=Array.isArray(conversation.messages)?conversation.messages.slice(-120):[];if(/already has an active writer/i.test(String(conversation.preview||""))){conversation.preview=utilityActiveWriterMessage();changed=true;}const firstUser=conversation.messages.find(message=>message.role==="user")?.text;if(firstUser&&/^Chat (Codex|Claude Code) ·/i.test(String(conversation.name||""))){conversation.name=utilityConversationTitle(firstUser,[],engine);changed=true;}}
      chat.messages=Array.isArray(chat.messages)?chat.messages.slice(-120).map(message=>{const text=friendlyUtilityError(message?.text);if(text!==message?.text)changed=true;return {...message,text,attachments:Array.isArray(message?.attachments)?message.attachments.map(normalizeUtilityAttachment).filter(Boolean).slice(0,utilityAttachmentLimit):[]}}):[];
      if(/already has an active writer/i.test(String(chat.lastRun?.summary||""))){chat.lastRun.summary=utilityActiveWriterMessage();changed=true;}
  ...
  const accessLabel = value => value==="workspaceWrite"?t("Modifications autorisées"):t("Analyse uniquement");
  ...
  function renderFirstRun(){
    if(!board.spaces.length)
      return `<div class="first-run"><span class="eyebrow">${t("PREMIER PAS")}</span><h2>${t("Commence par un projet.")}</h2><p class="lead">${t("Un projet relie un tableau à un dossier de ton Mac. C’est ce dossier que l’agent lira, et dans lequel il travaillera si tu l’y autorises.")}</p><div class="first-run-actions"><button class="ui-button primary" data-action="add-space">${t("Créer un projet")}</button></div></div>`;
    const etapes=[
  ...
  const backgroundSchedulerPresentation = () => {
    ...
    return {label:"Activation…",tone:"pending",description:"macOS installe le moteur local."};
  };
  ...
  function organisationSettingsMarkup(){
    ...
    const priorityChips=priorities().map(priority=>`<span style="--organisation-color:#${esc(priority.color)}"><i></i>${esc(priority.name)}</span>`).join("");
    const valueChips=values.slice(0,6).map(value=>`<span style="--organisation-color:#${esc(value.color||"6E75FF")}" title="${esc(value.dimension)}"><i></i>${esc(value.name)}</span>`).join("");
  ...
  function renderSettingsPage(){
    const dataPath="~/Library/Application Support/CTRL KANB/board.json";
    ...
    return `<div class="settings-shell"><main class="settings-content" id="settings-content"><form id="preferences-form" class="settings-page-form">
      ...
      <section id="settings-data" class="preference-section"><div class="preference-heading"><h2>${t("Données")}</h2><p>${t("Retrouve le fichier local utilisé par CTRL KANB et sa sauvegarde précédente.")}</p></div><div class="data-location"><div><strong>${t("Fichier principal")}</strong><code>${esc(dataPath)}</code></div><button type="button" class="secondary" data-action="data">${t("Afficher dans le Finder")}</button></div></section>
  ...
  function renderCard(card) {
    ...
    return `<article class="card ${blocked||waitingValidation?"blocked":""} ${failed?"failed":""} ${paused?"paused":""}" draggable="true" data-card="${esc(card.id)}" style="--space-color:#${esc(space.accentHex)}">
      <div class="card-top"><span class="priority-ref" style="--priority-color:#${esc(priority.color)}" title="${esc(priority.name)}">${esc(cardReference(card))}</span><h3><button class="card-title-button" data-action="edit-card" data-id="${esc(card.id)}">${esc(card.title)}</button></h3><button class="more pin-task ${card.pinned?'pinned':''}" data-action="pin-card" data-id="${esc(card.id)}" aria-label="${t("Épingler la tâche")}" aria-pressed="${Boolean(card.pinned)}">${icon("pin")}</button></div>
      ${selection==="global"||specialViews[selection]?`<div class="space-tag"><i></i>${esc(space.name)}</div>`:""}
      <p class="prompt">${esc(card.prompt)}</p>
      ${categories.length?`<div class="taxonomy-badges">${categories.map(({dimension,value})=>`<span style="--tag-color:#${esc(value.color||"6E75FF")}" title="${esc(dimension.name)}">${esc(dimension.name)} · ${esc(value.name)}</span>`).join("")}</div>`:""}
  ...
  function openSpaceModal(space=null) {
    modal(`<div class="project-editor-head"><span style="--project-color:#${esc(space?.accentHex||"6E75FF")}">${icon("folder")}</span><div><h2>${space?t("Modifier le projet"):t("Nouveau projet")}</h2><p class="lead">${space?t("Le nom et la couleur changent seulement l’affichage. Le dossier pilote les futures exécutions de ce projet."):t("Relie un tableau à un dossier local.")}</p></div></div><form id="space-form" class="form-grid" data-id="${esc(space?.id||"")}"><div class="two-cols"><div class="field"><label>${t("Nom du projet")}</label><input name="name" value="${esc(space?.name||"")}" autofocus required placeholder="${t("Mon projet")}"></div><div class="field"><label>${t("Couleur")}</label><input name="accentHex" type="color" value="#${esc(space?.accentHex||"6E75FF")}"></div></div><div class="field"><label>${t("Dossier de travail")}</label><div class="path-row"><input name="rootPath" value="${esc(space?.rootPath||"")}" required placeholder="${t("/chemin/du/projet")}"><button type="button" class="secondary" data-action="choose-folder">${t("Choisir…")}</button></div></div><label class="preference-toggle compact-project-toggle"><span><strong>${t("Épingler dans le panneau latéral")}</strong><small>${t("Le projet reste au-dessus des projets non épinglés.")}</small></span><input type="checkbox" name="pinned" ${space?.pinned?"checked":""}></label><div class="modal-actions"><button type="button" class="secondary" data-action="close-modal">${t("Annuler")}</button><button class="primary">${space?t("Enregistrer"):t("Créer le projet")}</button></div></form>`);
    ...
    <form id="card-form" class="task-editor-form simple-task-form" data-id="${esc(card?.id||"")}" data-simple="true">
      <div class="simple-task-body">
        ...
        <aside class="simple-task-options">
          ...
          <div class="field"><label>${t("Accès au projet")}</label><select name="runMode"><option value="readOnly" ${card?.runMode!=="workspaceWrite"?"selected":""}>${t("Analyse uniquement")}</option><option value="workspaceWrite" ${card?.runMode==="workspaceWrite"?"selected":""}>${t("Autoriser les modifications")}</option></select></div>
...
<details class="form-section" ${assignedValues(contentCard).length?"open":""}><summary>${t("Classement")} <small>${t("Objectifs, sous-projets et étiquettes")}</small></summary><div class="taxonomy-grid">${categoryFields(contentCard)}</div><div class="field"><label>${t("Étiquettes libres")}</label><input name="labels" value="${esc((contentCard?.labels||[]).join(", "))}" placeholder="${t("Séparées par des virgules")}"></div><div class="field"><label>${t("Numéro dans la priorité")}</label><input type="number" min="1" name="priorityNumber" value="${esc(contentCard?.priorityNumber||"")}" placeholder="${t("Attribué automatiquement")}"></div></details>
      <details class="form-section" ${card?.conversations?.length?"open":""}><summary>${t("Accès et conversations")} <small>${t("Droits de modification et historique de l’agent")}</small></summary><div class="field"><label>${t("Accès au projet")}</label><select name="runMode"><option value="readOnly" ${contentCard?.runMode!=="workspaceWrite"?"selected":""}>${t("Analyse uniquement")}</option><option value="workspaceWrite" ${contentCard?.runMode==="workspaceWrite"?"selected":""}>${t("Autoriser les modifications")}</option></select></div>${conversationPanelEditor(card)}</details>
  ...
  function modal(content,kind=""){
    ...
    requestAnimationFrame(()=>{const root=document.querySelector("#modal-root");root.querySelectorAll(".field").forEach((field,index)=>{const label=field.querySelector("label"),control=field.querySelector("input,select,textarea");if(label&&control&&!label.htmlFor){control.id||=`modal-field-${index}`;label.htmlFor=control.id;}});(root.querySelector("[autofocus]")||root.querySelector("input,textarea,select,button"))?.focus({preventScroll:true});});
  }
  ...
  const commandActions=()=>[
    ...
    {label:t("Terminal du projet"),detail:t("Exécuter des commandes dans le dossier choisi"),key:"⌥⌘T",icon:"terminal",run:()=>openUtilityPanel("terminal")},
  ...
  function securitySettingsMarkup(){
    const gesture=securityState.biometry||"le mot de passe de la session";
    return `<section id="settings-security" class="preference-section"><div class="preference-heading"><h2>${t("Sécurité")}</h2><p>${t("Protège l’accès à CTRL KANB sur ce Mac.")}</p></div><label class="preference-toggle"><span><strong>${t("Verrouiller CTRL KANB à l’ouverture")}</strong><small>${t("Demande {geste} avant d’afficher le plan de travail. Tant que l’écran de verrouillage est là, aucune donnée du Kanban n’est chargée.", {geste: esc(gesture)})}</small></span><input type="checkbox" data-action="toggle-app-lock" ${securityState.lockEnabled?"checked":""} ${securityState.checked?"":"disabled"}></label><div class="security-facts"><div><strong>${t("Ce que le verrou protège")}</strong><span>${t("L’accès à la fenêtre depuis une session déjà ouverte. Le réglage vit dans le trousseau : il ne se désactive pas en modifiant un fichier.")}</span></div><div><strong>${t("Ce qu’il ne protège pas")}</strong><span>${t("Le fichier lui-même.")} <code>board.json</code> ${t("est en lecture pour ton seul compte (0600) mais reste en clair : un programme lancé sous ton identité peut le lire.")}</span></div><div><strong>${t("Tâches programmées")}</strong><span>${t("Elles continuent en arrière-plan, fenêtre fermée. Le verrou s’applique quand tu rouvres la fenêtre.")}</span></div></div>${securityState.lockEnabled?`<button type="button" class="secondary" data-action="lock-now">Verrouiller maintenant <kbd>⌘ L</kbd></button>`:""}</section>`;
  }
  ...
  function renderUtilityFiles(space){
    ...
    return `<div class="utility-files-head"><div class="utility-breadcrumb"><button class="icon-button" data-action="browse-project-folder" data-path="${esc(parent)}" ${!state.path?"disabled":""} aria-label="${t("Dossier parent")}">${icon("previous")}</button><span title="${esc(state.path)}">${esc(where)}</span></div><button class="ui-button secondary" data-action="reveal-project-root">${icon("finder")}<span>Finder</span></button></div><div class="utility-file-list">${content}</div>${menu}`;
  }
  ...
  document.addEventListener("submit",event=>{
    ...
    if(form.id==="account-form"){
      ...
      const home=`${accountRoot}/${engine}/${id.split(":")[1]}`;
  ...
  window.CodexBoard={
    ...
    agentStatus(payload){engineAvailability={...payload,checked:true};if(selection==="settingsView")render();enhanceWorkspace();},
    ...
    agentProbeResult(payload){if(!payload?.engine)return;const cle=payload.account||activeAccountID(payload.engine),result={...payload,account:cle};engineProbes={...engineProbes,[cle]:result};persistAccountCheck(cle,result);if(selection==="settingsView")render();else enhanceWorkspace();},
    appInfo(payload){
    ...
    conversationsLoaded({conversations:items}){conversations=items||[];renderConversationPicker()},conversationsFailed({message}){modal(`<h2>Conversations indisponibles</h2><p class="lead">${esc(message)}</p><div class="modal-actions"><button class="secondary" data-action="close-modal">${t("Fermer")}</button></div>`)},
    ...
    terminalFailed({terminalID,message}){for(const terminal of utilityTerminals.values())if(terminal.id===terminalID){terminal.starting=false;terminal.running=false;terminal.output+=`\n${message}\n`;renderUtilityPanel();break}toast(message,true)},
    ...
    projectFilesFailed({spaceID,message}){utilityFileMenu=null;const state=utilityFiles(spaceID);state.loading=false;state.loaded=true;state.error=message||t("Impossible de lire ce dossier.");renderUtilityPanel()},
    projectFileFailed({message}){toast(message||t("Impossible d’ouvrir ce fichier."),true)}
  };
